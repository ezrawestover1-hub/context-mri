...
export function ContextPack({ contexts, report, applied, onApply, onCopy }: ContextPackProps) {
  ...
  return <section className={`context-pack ${applied ? 'applied' : ''}`}>
    <div className="pack-title"><span>RECOMMENDED CONTEXT PACK</span><small>{applied ? 'Applied to the next run' : 'Only the files that measurably help.'}</small></div>
    <div className="pack-provenance">Source: empirical ablation · {report.repeats} runs per removal</div>
