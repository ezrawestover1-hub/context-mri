...
The final eight-scene render uses the real deployed interface, burned-in captions, and the narration below. The preferred submission file is `submission/video/out/context-mri-demo-winner-pass.mp4`; the previously approved render remains preserved at `submission/video/out/context-mri-demo.mp4`.

## 0:00–0:15 — Hook

**Screen:** Context MRI title card, the 43 → 92 result, and the distinction judges must remember: most evals stop at a score; Context MRI finds the file, verifies the repair, and guards it in CI.
...
**Screen:** Diagnose, repair, and prevent: establish the baseline, remove one file while holding everything else constant, then verify and guard the repaired pack.
...
**Screen:** The harmful file diagnosis, baseline score, verified score, repeats, token reduction, and a short proof strip showing the same engine is configured for Support API, Billing API, and Release Security contracts.
...
**Screen:** Free public judge path first and visually dominant; the optional local GPT-5.6 Sol runner is secondary.
...
**Screen:** Final Context MRI card, public URL, Codex + GPT-5.6 build summary, 36 automated tests, independent evaluator, SHA-256 provenance, and Context Guard CI.
...
- Winner-pass render: verified H.264, 1920×1080, 30 fps, stereo AAC, 2:17.17 (well under three minutes), with the original render preserved unchanged.
...
- The winner-focused visual pass changes only on-screen hierarchy and proof. All approved human narration and caption timings remain untouched.
